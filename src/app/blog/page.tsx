import { AiOutlineRight } from "react-icons/ai";
import { AiOutlineArrowRight } from "react-icons/ai";
import { AiOutlineComment } from "react-icons/ai";
import { BiCalendar } from "react-icons/bi";
import { AiOutlinePhone } from "react-icons/ai";
import { BsSearch } from "react-icons/bs";

export default function page() {
  return (
    <>
      <div className="hero-container-contact">
        <div className="dottedimg">
          <img
            src="data:image/png;base64,"
            alt=""
          />
        </div>
        <div className="containt-text1">
          <h1 className="about-heading">Blog Standard</h1>
          <a href="" className="home-group1">
            Home
            <span className="left-iconleft">
              <AiOutlineRight />
            </span>
            <span className="linkabout">Blog Standard</span>
          </a>
        </div>
        <div className="contact-img-group-hero">
          <img
            src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAA4AAAAOCAYAAAAfSC3RAAAAVUlEQVQokWOMDHvJgAVMYGBgyIcKT2RgYChAV8KETRcxYAhpZIEGBDqwR+LbY1MDCtX/dA0cFmg8YXOqAZR9gYGB4SA2jRiRC/UTTCNI08hLAAwMDABc4wzgy1527AAAAABJRU5ErkJggg=="
            alt=""
          />
          <img
            src="data:image/png;base64,"
            alt=""
          />
        </div>
      </div>
      {/* blog-section */}
      <div className="container-blog">
        <div className="img-blog-parent">
          <img
            src="https://wordpressriverthemes.com/htmltemp/pixlab/assets/images/blog/standard-1.jpg"
            alt=""
          />
          <div className="content-first-blog">
            <div className="icon-blog-gropline">
              <a href="">Web design</a>
              <span className="calender-blog-icon">
                <BiCalendar />
                <a href="">22 March 2022</a>
              </span>
              <span className="calender-blog-icon">
                <AiOutlineComment />
                <a href="">Comment (5)</a>
              </span>
            </div>

            <h3 className="tittleline">
              <a href="" className="titlrtext">
                Powerful Terminal And Command Line Seeny
                <br />
                Tools Modern Web Development
              </a>
            </h3>
            <p>
              Sit amet consectetur adipiscing elit sed do eiusmod temp didunt ut
              labore et dolore magna aliqua suspendisse
            </p>
            <a href="" className="readbtn">
              Read More
              <span className="readbtnicon">
                <AiOutlineRight />
              </span>
            </a>
          </div>
        </div>
        <div className="blog-sidebar">
          <div className="blog-text-category">
            <input
              type="text"
              className="blog-input"
              placeholder="Search here"
            />
            <span className="search-blog">
              <BsSearch />
            </span>
            <div className="Category-card">
              <h4>Category</h4>
              <span className="line"></span>
              <div className="blog-text-icon-main">
                <a href="">Organic Vegetables</a>
                <span className="arro-icon-blog">
                  <AiOutlineArrowRight />
                </span>
              </div>
              <div className="blog-text-icon-main">
                <a href="">Fresh Jack Fruits</a>
                <span className="arro-icon-blog">
                  <AiOutlineArrowRight />
                </span>
              </div>
              <div className="blog-text-icon-main">
                <a href="">Chicken Meat & Eggs</a>
                <span className="arro-icon-blog">
                  <AiOutlineArrowRight />
                </span>
              </div>
              <div className="blog-text-icon-main">
                <a href="">Organic Wheats</a>
                <span className="arro-icon-blog">
                  <AiOutlineArrowRight />
                </span>
              </div>
              <div className="blog-text-icon-main">
                <a href="">Cow Meat & Milk</a>
                <span className="arro-icon-blog">
                  <AiOutlineArrowRight />
                </span>
              </div>
            </div>
          </div>
          <div className="img-blog-contact">
            <div className="Category-card1">
              <div className="phone-blog-icon-main">
                <div className="phone-blog-icon">
                  <div className="icon-phone-blog">
                    <AiOutlinePhone />
                  </div>
                </div>
              </div>
              <div className="bog-contact-textme">
                <a href="">+919170065975</a>
                <a href="">sidzifi@gmail.com</a>
                <a href="" className="contactbtn-me">
                  Contact Us
                </a>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="container-blog">
        <div className="img-blog-parent">
          <img
            src="https://wordpressriverthemes.com/htmltemp/pixlab/assets/images/blog/standard-2.jpg"
            alt=""
          />
          <div className="content-first-blog">
            <div className="icon-blog-gropline">
              <a href="">Web design</a>
              <span className="calender-blog-icon">
                <BiCalendar />
                <a href="">22 March 2022</a>
              </span>
              <span className="calender-blog-icon">
                <AiOutlineComment />
                <a href="">Comment (5)</a>
              </span>
            </div>

            <h3 className="tittleline">
              <a href="" className="titlrtext">
                Everything You Want To Know About Creating Voice User Interfaces
                Powerful Terminal
              </a>
            </h3>
            <p>
              Sit amet consectetur adipiscing elit sed do eiusmod temp didunt ut
              labore et dolore magna aliqua suspendisse
            </p>
            <a href="" className="readbtn">
              Read More
              <span className="readbtnicon">
                <AiOutlineRight />
              </span>
            </a>
          </div>
        </div>
        <div className="blog-sidebar">
          <div className="blog-text-category">
            <div className="Category-card">
              <h4>Recent News</h4>
              <span className="line"></span>
              <div className="blog-text-icon-main1">
                <img
                  src="https://wordpressriverthemes.com/htmltemp/pixlab/assets/images/widget/news-1.jpg"
                  alt=""
                />
                <a href="">
                  Guid Mordern CSS Colors HWB,LAB
                  <p className="pargraph-blog">By Somalia D. Silba</p>
                </a>
              </div>
              <div className="blog-text-icon-main1">
                <img
                  src="https://wordpressriverthemes.com/htmltemp/pixlab/assets/images/widget/news-2.jpg"
                  alt=""
                />
                <a href="">
                  Guid Mordern CSS Colors HWB,LAB
                  <p className="pargraph-blog">By Somalia D. Silba</p>
                </a>
              </div>
            </div>
          </div>
          <div className="Category-card">
            <h4>Popular Tags</h4>
            <span className="line"></span>
            <div className="btnblog-group">
              <a href="" className="creative-btn">
                3D Creative
              </a>
              <a href="" className="creative-btn">
                Agency
              </a>
            </div>
            <div className="btnblog-group">
              <a href="" className="creative-btn">
                Digital
              </a>
              <a href="" className="creative-btn">
                Web Design
              </a>
              <a href="" className="creative-btn">
                UI/UX
              </a>
            </div>
            <div className="btnblog-group">
              <a href="" className="creative-btn">
                Strategy
              </a>
            </div>
            {/* <div className="blog-text-icon-main">
              <a href="">Organic Vegetables</a>
              <span className="arro-icon-blog">
                <AiOutlineArrowRight />
              </span>
            </div>
            <div className="blog-text-icon-main">
              <a href="">Fresh Jack Fruits</a>
              <span className="arro-icon-blog">
                <AiOutlineArrowRight />
              </span>
            </div>
            <div className="blog-text-icon-main">
              <a href="">Chicken Meat & Eggs</a>
              <span className="arro-icon-blog">
                <AiOutlineArrowRight />
              </span>
            </div>
            <div className="blog-text-icon-main">
              <a href="">Organic Wheats</a>
              <span className="arro-icon-blog">
                <AiOutlineArrowRight />
              </span>
            </div>
            <div className="blog-text-icon-main">
              <a href="">Cow Meat & Milk</a>
              <span className="arro-icon-blog">
                <AiOutlineArrowRight />
              </span>
            </div> */}
          </div>
          {/* <div className="img-blog-contact">
            <div className="Category-card1">
              <div className="phone-blog-icon-main">
                <div className="phone-blog-icon">
                  <div className="icon-phone-blog">
                    <AiOutlinePhone />
                  </div>
                </div>
              </div>
              <div className="bog-contact-textme">
                <a href="">+919170065975</a>
                <a href="">sidzifi@gmail.com</a>
                <a href="" className="contactbtn-me">
                  Contact Us
                </a>
              </div>
            </div>
          </div> */}
        </div>
      </div>
      <div className="container-blog">
        <div className="img-blog-parent">
          <img
            src="https://wordpressriverthemes.com/htmltemp/pixlab/assets/images/blog/standard-3.jpg"
            alt=""
          />
          <div className="content-first-blog">
            <div className="icon-blog-gropline">
              <a href="">Web design</a>
              <span className="calender-blog-icon">
                <BiCalendar />
                <a href="">22 March 2022</a>
              </span>
              <span className="calender-blog-icon">
                <AiOutlineComment />
                <a href="">Comment (5)</a>
              </span>
            </div>

            <h3 className="tittleline">
              <a href="" className="titlrtext">
                Powerful Terminal And Command Line Seeny
                <br />
                Tools Modern Web Development
              </a>
            </h3>
            <p>
              Sit amet consectetur adipiscing elit sed do eiusmod temp didunt ut
              labore et dolore magna aliqua suspendisse
            </p>
            <a href="" className="readbtn">
              Read More
              <span className="readbtnicon">
                <AiOutlineRight />
              </span>
            </a>
          </div>
        </div>
        {/* <div className="blog-sidebar">
          <div className="blog-text-category">
            <input
              type="text"
              className="blog-input"
              placeholder="Search here"
            />
            <span className="search-blog">
              <BsSearch />
            </span>
            <div className="Category-card">
              <h4>Category</h4>
              <span className="line"></span>
              <div className="blog-text-icon-main">
                <a href="">Organic Vegetables</a>
                <span className="arro-icon-blog">
                  <AiOutlineArrowRight />
                </span>
              </div>
              <div className="blog-text-icon-main">
                <a href="">Fresh Jack Fruits</a>
                <span className="arro-icon-blog">
                  <AiOutlineArrowRight />
                </span>
              </div>
              <div className="blog-text-icon-main">
                <a href="">Chicken Meat & Eggs</a>
                <span className="arro-icon-blog">
                  <AiOutlineArrowRight />
                </span>
              </div>
              <div className="blog-text-icon-main">
                <a href="">Organic Wheats</a>
                <span className="arro-icon-blog">
                  <AiOutlineArrowRight />
                </span>
              </div>
              <div className="blog-text-icon-main">
                <a href="">Cow Meat & Milk</a>
                <span className="arro-icon-blog">
                  <AiOutlineArrowRight />
                </span>
              </div>
            </div>
          </div>
          <div className="img-blog-contact">
            <div className="Category-card1">
              <div className="phone-blog-icon-main">
                <div className="phone-blog-icon">
                  <div className="icon-phone-blog">
                    <AiOutlinePhone />
                  </div>
                </div>
              </div>
              <div className="bog-contact-textme">
                <a href="">+919170065975</a>
                <a href="">sidzifi@gmail.com</a>
                <a href="" className="contactbtn-me">
                  Contact Us
                </a>
              </div>
            </div>
          </div>
        </div> */}
      </div>
    </>
  );
}
